...
export async function analyze(
  ...
  const system =
    ...
    "project — BREAK IT DOWN: use the brief, marking criteria/rubric, attachments and syllabus so 'plan' is a thorough, ordered list of concrete actionable steps from understanding the task to final submission, mapped to the requirements/marking criteria; 'requirements' lists exactly what to deliver. A project is PRODUCED, not memorised, so return flashcards:[] and leave revision.practiceQuestions and revision.examQuestions as [] — UNLESS the notification explicitly requires the student to LEARN or PRESENT content from memory (oral presentation, viva, speech or a knowledge/test component), in which case add flashcards/practice for EXACTLY that content only.";
